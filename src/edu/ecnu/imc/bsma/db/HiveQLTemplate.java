...
public class HiveQLTemplate {
	public static final String QUERY1 = "SELECT f1.uid, COUNT(f2.uid) num FROM "
			+ "(SELECT a.followeeID AS uid "
			+ "FROM friendList a JOIN friendList b "
			+ "ON (a.followeeID = b.uid) "
			+ "WHERE a.uid = b.followeeID AND a.uid = '%s')  f JOIN friendList f2 ON (f2.uid = f.uid) "
