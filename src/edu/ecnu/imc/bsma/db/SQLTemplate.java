...
public class SQLTemplate {
	public static final String QUERY1 = "SELECT f1.uid, count(*) as num FROM "
			...
			+ "AND f1.uid <> %$1s "
			+ "and f1.uid not in (select followeeID from friendList where uid =%$1s) "
			+ "GROUP BY f1.uid ORDER BY num DESC;";
	
	public static final String QUERY2 = "SELECT  followers.uid, count(followers.uid) as num "
			+ " FROM (SELECT followeeID  FROM  friendList  WHERE  uid = \"%s\") "
			+ " friends LEFT OUTER JOIN (SELECT f1.uid, f1.followeeID "
			+ " FROM  (SELECT followeeID  FROM  friendList  WHERE  uid = \"%1$s\")"
